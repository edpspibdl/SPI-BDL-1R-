...
//".$condition."
		$ora= oci_parse($conn,"
				select * from PHP_REKOMENDASI_SECOND where plu='1349680'
		...
		while (($row = oci_fetch_array($ora, OCI_NUM)) != false) {
			...
			//PLU='".$row[0]."' 
			$ora2= oci_parse($conn,"
				select * from PHP_REKOMENDASI_FIRST WHERE 
				
				PLU='".$row[0]."' 
				
				and substr(exp_date,4,6)='".$row[1]."'  order by maxpallet_min_plano desc");
      ...
      <tbody>
          ...
          <?php
			...
			for ($a=0; $a<count($data); $a++){
				...
				echo '$qty_plano_client'.$qty_plano_client;
				$qty_kirim=0;
