...
$query = "SELECT
  trn_type,
  SUBSTRING(trn_tgldoc::text FROM 1 FOR 10) AS trn_tgldoc,
  trn_nodoc,
  trn_nopo,
  SUBSTRING(trn_tglpo::text FROM 1 FOR 10) AS trn_tglpo,
  trn_div,
  trn_dept,
  trn_katb,
  trn_prdcd,
  ...
  trn_tag,
  trn_qty,
  coalesce(trn_qty_bonus1, 0) + coalesce(trn_qty_bonus2, 0) AS trn_qty_bonus,
  trn_harga_satuan,
  trn_gross,
  trn_discount,
  trn_ppn,
...
FROM (
  ...
  FROM
    tbtr_mstran_d AS m
  LEFT JOIN
    tbmaster_prodmast AS p
    ON m.mstd_prdcd = p.prd_prdcd
  LEFT JOIN
    tbmaster_supplier AS s
    ON m.mstd_kodesupplier = s.sup_kodesupplier
  WHERE
...
<section class="section">
    <div class="section-header d-flex justify-content-between">
        <h1>History BackOffice Produk Detail</h1>
    </div>
    <div class="row">
        <div class="col-12">
            <div class="card">
                        ...
                        <table class="table table-hover table-striped" id="table-1">
                            <thead>
                                <tr class="info">
                                    ...
                                    <th colspan="2" class="text-center">PO/Referensi</th>
                            ...
                            <tbody>
                                ...
                                if (empty($data)) :
                                    echo '<tr><td colspan="25" class="text-center">Tidak ada data</td></tr>'; // Updated colspan
                                else :
                                    foreach ($data as $row) :
                                        ...
                                        // Dokumen
                                        echo '<td align="center">' . htmlspecialchars($row['trn_nodoc']) . '</td>';
                                        echo '<td align="center" class="text-nowrap">' . htmlspecialchars($row['trn_tgldoc']) . '</td>';

                                        // PO/Referensi
                                        echo '<td align="center">' . htmlspecialchars($row['trn_nopo']) . '</td>';
                                        echo '<td align="center" class="text-nowrap">' . htmlspecialchars($row['trn_tglpo']) . '</td>';
                                        ...
                                        echo '<td align="left" class="text-nowrap">' . htmlspecialchars($row['trn_nama_barang']) . '</td>';
                                        ...
                                        // Rupiah
                                        echo '<td align="right">' . number_format($row['trn_harga_satuan'], 0, '.', ',') . '</td>'; // Assuming trn_harga_satuan is the 'Harga'
                                        ...
                                        echo '<td align="left" class="text-nowrap">' . htmlspecialchars($row['trn_nama_supplier']) . '</td>';

                                        // Keterangan
                                        $keterangan = ' ';
                                        if ($row['trn_type'] == 'Z') { // Changed 'z' to 'Z' assuming it's uppercase
                                            switch ($row['trn_flag1']) {
                                                case "B":
                                                    $keterangan = 'Baik';
                                                    break;
                                                case "T":
                                        ...
                                        } else if (isset($row['keterangan'])) { // Added a check for 'keterangan' from SQL
                                            $keterangan = $row['keterangan'];
                                        }
                                        echo '<td align="left" class="text-nowrap">' . htmlspecialchars($keterangan) . '</td>';
                            ...
                            <tfoot>
                                <tr>
                                    <td colspan="19" class="text-right font-weight-bold">TOTAL</td>
