<?php
require_once '../helper/connection.php'; // Menggunakan $conn dari file ini
...
try {
          if (!isset($conn) || !$conn instanceof PDO) {
                    throw new Exception("Koneksi PDO tidak ditemukan atau tidak valid.");
          }

          $sql = "SELECT
        COALESCE(CAST(SUM(dtl_netto) AS BIGINT), 0) AS earned_this_month
    FROM (
        SELECT
            dtl_rtype,
            dtl_tanggal,
    ...
    WHERE DATE_TRUNC('month', dtl_tanggal) = DATE_TRUNC('month', current_date )";
          ...
          $stmt = $conn->prepare($sql); // ← pakai $conn sesuai connection.php
          $stmt->execute();

          $result = $stmt->fetch(PDO::FETCH_ASSOC);

          if ($result && array_key_exists('earned_this_month', $result)) {
                    $response['success'] = true;
                    $response['earned_this_month'] = (float) $result['earned_this_month'];
                    $response['message'] = "Data pendapatan berhasil diambil.";
          } else {
                    $response['success'] = true;
                    $response['earned_this_month'] = 0;
                    $response['message'] = "Tidak ada data pendapatan ditemukan.";
          }
