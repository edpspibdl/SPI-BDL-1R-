...
ini_set('display_errors', 1); // Aktifkan ini untuk debugging di lingkungan pengembangan
ini_set('display_startup_errors', 1);
error_reporting(E_ALL);

$response = [
          'success' => false,
          'gross_this_month' => 0, // Nama kunci respons diubah
          'message' => ''
];
...
try {
          ...
          $sql = "SELECT
                COALESCE(CAST(SUM(dtl_gross) AS BIGINT), 0) AS gross_this_month  -- Mengambil SUM dari dtl_gross
            FROM (
                SELECT
                    dtl_rtype,
                    dtl_tanggal,
                    ...
                    dtl_diskon,
                    CASE
                        WHEN dtl_rtype = 'S' THEN dtl_gross_original  -- Menggunakan dtl_gross_original
          ...
          if ($result) {
                    ...
                    // Pastikan tipe data sesuai, bisa float atau int tergantung kebutuhan
                    $response['gross_this_month'] = (float) $result['gross_this_month'];
          } else {
                    $response['message'] = "No gross sales data found for this month.";
          }
} catch (PDOException $e) {
          http_response_code(500);
          $response['message'] = "Database error: " . $e->getMessage();
          error_log("PDO Error in gross_berjalan.php: " . $e->getMessage());
} catch (Exception $e) {
          http_response_code(500);
          $response['message'] = "Server error: " . $e->getMessage();
          error_log("General Error in gross_berjalan.php: " . $e->getMessage());
}
