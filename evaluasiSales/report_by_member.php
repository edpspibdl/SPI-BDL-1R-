...
FROM ( SELECT
    ...
    CASE
        WHEN dtl_rtype = 'S' THEN
            dtl_netto - dtl_hpp
        ELSE
            ( dtl_netto - dtl_hpp ) * - 1 END AS dtl_margin,
    dtl_k_div,
    dtl_nama_div,
    dtl_k_dept,
    dtl_nama_dept,
...
FROM ( SELECT
    ...
    END AS dtl_qty_pcs,
    CASE
        WHEN trjd_flagtax1 = 'Y' AND trjd_create_by IN (
            'IDM',
            'OMI',
            'BKL'
        ) THEN
    ...
    ELSE 0 END ) ELSE trjd_nominalamt END ELSE trjd_nominalamt - ( CASE WHEN substr(trjd_create_by, 1, 2) = 'EX' 
    THEN 0 ELSE CASE WHEN trjd_flagtax1 = 'Y' AND coalesce(trjd_flagtax2, 'z') IN ( 'Y', 'z' ) AND coalesce(prd_kodetag, 'zz') <> 'Q' 
    ...
    ELSE ( prd_markupstandard * trjd_nominalamt ) / 100 END ) END 
    ELSE ( trjd_quantity / CASE WHEN prd_unit = 'KG' THEN 1000 ELSE 1 END ) * trjd_baseprice 
    END AS dtl_hpp FROM tbtr_jualdetail LEFT JOIN tbmaster_prodmast ON trjd_prdcd = prd_prdcd LEFT JOIN tbmaster_tokoigr 
    ON trjd_cus_kodemember = tko_kodecustomer LEFT JOIN tbmaster_customer ON trjd_cus_kodemember = cus_kodemember 
    LEFT JOIN tbmaster_customercrm ON trjd_cus_kodemember = crm_kodemember LEFT JOIN tbmaster_divisi ON trjd_division = div_kodedivisi 
    ...
    WHERE m.hgb_tipe = '2' AND m.hgb_recordid IS NULL ) gb ON dtl_prdcd_ctn = hgb_prdcd LEFT 
    JOIN ( SELECT jh_cus_kodemember, date_trunc('day', MIN(jh_transactiondate)) AS jh_belanja_pertama, date_trunc('day', MAX(jh_transactiondate)) AS jh_belanja_terakhir 
    FROM tbtr_jualheader WHERE jh_cus_kodemember IS NOT NULL GROUP BY jh_cus_kodemember ) blj ON dtl_cusno = jh_cus_kodemember ) z 
    WHERE to_char(dtl_tanggal, 'yyyymmdd') BETWEEN :tanggalMulai and :tanggalSelesai ----------sesuaikan periode
    AND DTL_PRDCD_CTN NOT IN (SELECT NON_PRDCD FROM TBMASTER_PLUNONPROMO ) 
    GROUP BY dtl_outlet, dtl_suboutlet, dtl_cusno, dtl_namamember having coalesce(SUM(dtl_netto),0) <> 0 
    ORDER BY dtl_outlet,
			          dtl_suboutlet,
			          dtl_cusno,
					  dtl_namamember
...
<section class="section">
    <div class="section-header d-flex justify-content-between">
        <h3 class="text-center">REPORT SALES DI LUAR ITEM LARANGAN BY MEMBER</h3>
                        ...
                        <table class="table table-hover table-striped" id="table-1">
                            <thead>
                                <tr>
                                    ...
                                    <th colspan="2" class="text-center">Outlet</th>
                                    <th colspan="2" class="text-center">Member</th>
                                    <th colspan="8" class="text-center">Sales</th>
                                    <th colspan="2" class="text-center">Tanggal Kunjungan</th>
                                </tr>
                                <tr>
                                    <th>Outlet</th>
                                    <th>Sub Outlet</th>
                            ...
                            <tbody>
                                <?php 
                                $noUrut = 1;
                                $totalKunjungan = $totalStruk = $totalProduk = $totalQtyInPcs = $totalGross = $totalNetto = $totalMargin = 0;

                                foreach ($result as $row): ?>
                                    <tr>
                                        <td align="right"><?= $noUrut++ ?></td>
                                        <td align="center"><?= $row['dtl_outlet'] ?></td>
                                        <td align="center"><?= $row['dtl_suboutlet'] ?></td>
                                        <td align="right"><?= $row['dtl_cusno'] ?></td>
                                        <td align="right"><?= $row['dtl_namamember'] ?></td>
                                        <td align="right"><?= number_format($row['kunjungan'], 0, '.', ',') ?></td>
                                        <td align="right"><?= number_format($row['struk'], 0, '.', ',') ?></td>
                                        <td align="right"><?= number_format($row['produk'], 0, '.', ',') ?></td>
                                        ...
                                        <td align="right"><?= $row['dtl_belanja_pertama'] ?></td>
                                        <td align="right"><?= $row['dtl_belanja_terakhir'] ?></td>
                                    </tr>
                                    <?php
                                    $totalKunjungan += $row['kunjungan'];
                                    $totalStruk += $row['struk'];
                                    $totalProduk += $row['produk'];
