...
$query = " SELECT
    ...
    CASE
        WHEN substr(hdr.obi_recid, 1, 1) = '1' THEN 'Siap Picking'
        WHEN substr(hdr.obi_recid, 1, 1) = '2' THEN 'Siap Packing'
        WHEN substr(hdr.obi_recid, 1, 1) = '3' THEN 'Siap Draft Struk'
        WHEN substr(hdr.obi_recid, 1, 1) = '4' THEN 'Konfirmasi Pembayaran'
        WHEN substr(hdr.obi_recid, 1, 1) = '5' THEN 'Siap Struk'
        WHEN substr(hdr.obi_recid, 1, 1) = '6' THEN 'Selesai Struk'
        WHEN substr(hdr.obi_recid, 1, 1) = '7' THEN 'Set Ongkir'
        WHEN substr(hdr.obi_recid, 1, 1) = 'B' THEN 'Transaksi Batal'
        ELSE 'Unknown'
    END AS status,
    rak_toko,
    rak_dpd,
    ...
    ROUND(sat_gram / COALESCE(sat_pcs, 1)) AS konversi
FROM
    tbtr_obi_h hdr
JOIN
...
LEFT JOIN
    (SELECT
     ...
     GROUP BY
         lks_prdcd,
         lks_koderak || '.' || lks_kodesubrak || '.' || lks_tiperak || '.' || lks_shelvingrak || '.' || lks_nourut) loc_toko ON substr(dtl.obi_prdcd, 1, 6) || '0' = loc_toko.PRDCD_TKO
LEFT JOIN
    (SELECT
     ...
     GROUP BY
         lks_prdcd,
         lks_koderak || '.' || lks_kodesubrak || '.' || lks_tiperak || '.' || lks_shelvingrak || '.' || lks_nourut) loc_dpd ON substr(dtl.obi_prdcd, 1, 6) || '0' = loc_dpd.PRDCD_DPD
WHERE
